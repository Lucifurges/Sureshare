...
def generate_token(email, password):
    ...
    try:
        ...
        if response.status_code == 200 and 'access_token' in data:
            ...
            if "Invalid credentials" in data['error']['message']:
                console.print("[-] Invalid credentials. Please check your email and password.", style="red")
            else:
                console.print(f"[-] Unknown error: {data['error']['message']}", style="red")
        else:
